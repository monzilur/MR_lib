...
[melbank.x, melbank.mc, melbank.na, melbank.nb] =  melbankbw(params.n_f, t_window_bins*params.nfft_mult, params.fs, params.f_min/params.fs, params.f_max/params.fs, params.meltype);        
...
[spec, freqs, t] = spectrogram(x, t_window_bins, t_overlap_bins, t_window_bins*params.nfft_mult, params.fs);

X_ft = nan(size(params.melbank.x,1), size(spec,2));
for tt = 1:size(spec,2)
    X_ft(:,tt)=max(10.*log10(params.melbank.x*abs(spec(params.melbank.na:params.melbank.nb,tt))), params.threshold);
    %X_ft(:,tt)=max(20.*log10(params.melbank.x*abs(spec(params.melbank.na:params.melbank.nb,tt))), params.threshold);
